...
router.post('/', async (req: AuthRequest, res) => {
    try {
        const { symbol, targetPrice, condition, amount, type, smartConditions }: CreateTriggerRequest = req.body;
        ...
        // For smart triggers, targetPrice can be 0
        if (!smartConditions && targetPrice <= 0) {
            return res.status(400).json({ error: 'Target price must be positive for simple triggers' });
        }
        ...
        const trigger = await prisma.trigger.create({
            data: {
                ...
                amount,
                type,
                smartConditions: smartConditions || null,
                status: 'ACTIVE'
            }
        });
