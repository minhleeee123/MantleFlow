...
const MessageItem: React.FC<MessageItemProps> = ({ msg, theme = 'dark' }) => {
  return (
    <div className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''} mb-6`}>
      ...
      {/* Avatar */}
      <div className={`w-10 h-10 border-2 border-black dark:border-white flex items-center justify-center shrink-0 rounded-xl
      ...
      <div className={`flex flex-col max-w-[90%] md:max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
        {msg.text && (
          <div className={`p-4 border-2 border-black dark:border-white text-[15px] leading-relaxed font-medium rounded-xl 
            ${msg.role === 'user'
