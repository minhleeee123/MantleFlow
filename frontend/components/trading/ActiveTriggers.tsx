...
const ActiveTriggers: React.FC<Props> = ({ triggers, onCancel }) => {
    return (
        <div className="bg-white dark:bg-[#1e1f20] border-2 border-black dark:border-white shadow-neo p-6 flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
                {triggers.length === 0 ? (
                    ...
                    triggers.map(trigger => (
                        <div key={trigger.id} className="relative group bg-gray-50 dark:bg-black border-2 border-black dark:border-gray-700 p-3 transition-all hover:bg-white dark:hover:bg-[#111] hover:shadow-neo-sm">
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                    <span className={`text-[10px] font-black px-1.5 py-0.5 uppercase border border-black ${trigger.type === 'BUY' ? 'bg-neo-secondary text-black' : 'bg-neo-accent text-black'}`}>
                                        {trigger.type}
                                    </span>
                                ...
                                <button
                                    ...
                                    className="p-1.5 hover:bg-red-500 hover:text-white border border-black dark:border-gray-600 transition-colors"
                            ...
                            <div className="absolute top-1/2 -right-1 transform translate-x-full opacity-0 group-hover:opacity-100 transition-all">
                                <div className="flex items-center gap-1 bg-black text-white text-[10px] px-2 py-1">
