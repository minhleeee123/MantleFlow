...
import { getMockPrice, getMockRSI, getMockVolume, getMockMA, getMockSentiment, getMockGas } from '../../services/data/mockDataSources';
    ...
    useEffect(() => {
        ...
        if (trigger.smartConditions && trigger.smartConditions.length > 0) {
            ...
            const runSmartCheck = async () => {
                ...
                // Check all conditions using Mock APIs
                for (const cond of trigger.smartConditions!) {
                    ...
                    switch (cond.metric) {
                        case 'PRICE': 
                            realValue = await getMockPrice(trigger.symbol); 
                            fetchedPrice = realValue;
                            break;
                        case 'RSI': realValue = await getMockRSI(trigger.symbol); break;
                        case 'VOLUME': realValue = await getMockVolume(trigger.symbol); break;
                        case 'MA': realValue = await getMockMA(trigger.symbol); break;
                        case 'SENTIMENT': realValue = await getMockSentiment(); break;
                        case 'GAS': realValue = await getMockGas(); break;
                    }
                    ...
                    newLogs.push({
                        ...
                        metric: cond.metric,
                        realValue: cond.metric === 'PRICE' || cond.metric === 'MA' ? `$${realValue.toLocaleString()}` : realValue.toString(),
