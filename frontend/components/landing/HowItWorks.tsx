            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
                {steps.map((step, idx) => (
                    <div key={idx} className="relative group h-full">
                        ...
                        <div className="bg-white dark:bg-[#1a1a1a] border-2 border-black dark:border-white p-8 relative z-10 shadow-neo group-hover:translate-x-[-4px] group-hover:translate-y-[-4px] group-hover:shadow-neo-lg transition-all duration-300 h-full flex flex-col">
                            <div className={`w-16 h-16 ${step.color} border-2 border-black flex items-center justify-center mb-6 shadow-neo-sm transform group-hover:rotate-6 transition-transform shrink-0`}>
                                <step.icon className="w-8 h-8 text-black" strokeWidth={2.5} />
                            </div>
                            
                            <div className="absolute top-4 right-4 text-6xl font-black text-gray-100 dark:text-gray-800 pointer-events-none select-none">
                                0{step.id}
                            </div>
